...
/*
        ...
        TODO :	currently support only rgb images in which width*bytes_per_pixel = stride OR
                        ...
                        stride should be > 0
*/
__declspec(dllexport) int lz_encode(LzContext *lz, LzImageType type, int width, int height, int top_down,
...
*/
__declspec(dllexport) void lz_decode_begin(LzContext *lz, uint8_t *io_ptr, unsigned int num_io_bytes,
...
*/
__declspec(dllexport) void lz_decode(LzContext *lz, LzImageType to_type, uint8_t *buf);

__declspec(dllexport) LzContext *lz_create(LzUsrContext *usr);

__declspec(dllexport) void lz_destroy(LzContext *lz);

__declspec(dllexport) LzUsrContext* lz_create_usr_context();
__declspec(dllexport) void lz_destroy_usr_context(LzUsrContext* usr);
