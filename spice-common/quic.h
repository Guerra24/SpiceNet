...
__declspec(dllexport) int quic_encode(QuicContext* quic, QuicImageType type, int width, int height,
    uint8_t* lines, unsigned int num_lines, int stride,
    uint32_t* io_ptr, unsigned int num_io_words);

__declspec(dllexport) int quic_decode_begin(QuicContext* quic, uint32_t* io_ptr, unsigned int num_io_words,
    QuicImageType* type, int* width, int* height);
__declspec(dllexport) int quic_decode(QuicContext* quic, QuicImageType type, uint8_t* buf, int stride);

__declspec(dllexport) QuicContext* quic_create(QuicUsrContext* usr);
__declspec(dllexport) void quic_destroy(QuicContext* quic);

__declspec(dllexport) QuicUsrContext* quic_create_usr_context();
__declspec(dllexport) void quic_destroy_usr_context(QuicUsrContext* usr);
